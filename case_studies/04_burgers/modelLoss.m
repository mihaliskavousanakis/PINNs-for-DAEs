...
A=repmat(AB(1,:),599,1); A=reshape(A,1,599*300);
B=repmat(AB(2,:),599,1); B=reshape(B,1,599*300);
f1=Ut-0.025*Uxx+U.*Ux-A.*(CP(2,:).*Ux+U)-B.*Ux;
...
% Template Condition
U=reshape(U,599,300); U=U';

Y=reshape(CP(2,:),599,300); Y=Y';
Temp=exp(-Y.^2);
dTemp=-2*Y.*exp(-Y.^2);

dx=CP(2,2)-CP(2,1);
f4=extractdata(dx)*sum((U-Temp).*(Temp+dTemp.*Y),2)'; f4=dlarray(f4,"CB");
zeroTarget=zeros(size(f4),"like",f4);
mseF5 = l2loss(f4,zeroTarget);

f5=extractdata(dx)*sum((U-Temp).*dTemp,2)'; f5=dlarray(f5,"CB");
